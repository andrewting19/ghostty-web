...
export class Terminal implements ITerminalCore {
  ...
  private timeOperation<T>(
    ...
    try {
      ...
      if (durationMs >= warnThresholdMs && this.isDebugLoggingEnabled()) {
        console.warn(
          `[ghostty][t${this.instanceId}] ${event} took ${durationMs.toFixed(1)}ms`,
          info
        );
      }
    }
  ...
  private writeInternal(data: string | Uint8Array, callback?: () => void): void {
    ...
    this.timeOperation(
      ...
      () => {
        ...
        // Handle OSC 10/11/12 color queries in JS. The web embedding already
        // knows the configured theme colors, and answering here avoids noisy
        // WASM warnings for allocator-dependent query paths.
        const nextData = this.interceptDynamicColorQueries(data);
        if (
          (typeof nextData === 'string' && nextData.length === 0) ||
          (nextData instanceof Uint8Array && nextData.byteLength === 0)
        ) {
        ...
        if (typeof nextData === 'string' && nextData.includes('\x07')) {
          this.bellEmitter.fire();
        } else if (nextData instanceof Uint8Array && nextData.includes(0x07)) {
          this.bellEmitter.fire();
        }
        ...
        if (typeof nextData === 'string' && nextData.includes('\x1b]')) {
          this.checkForTitleChange(nextData);
          this.checkForClipboardWrite(nextData);
        }
  ...
  private interceptDynamicColorQueries(data: string | Uint8Array): string | Uint8Array {
    const text = typeof data === 'string' ? data : new TextDecoder().decode(data);
    if (!text.includes('\x1b]1')) return data;

    const oscQueryRegex = /\x1b\](10|11|12);\?(\x07|\x1b\\)/g;
    const responses: string[] = [];
    const sanitized = text.replace(oscQueryRegex, (_match, code: string, terminator: string) => {
      const response = this.buildDynamicColorQueryResponse(code, terminator);
      if (response) responses.push(response);
      return '';
    });

    if (responses.length === 0) return data;
    for (const response of responses) {
      this.dataEmitter.fire(response);
    }

    return typeof data === 'string' ? sanitized : new TextEncoder().encode(sanitized);
  }

  private buildDynamicColorQueryResponse(code: string, terminator: string): string | null {
    ...
    const formatted = this.formatOscColorValue(color);
    return formatted ? `\x1b]${code};${formatted}${terminator}` : null;
  }

  private formatOscColorValue(color: string | undefined): string | null {
